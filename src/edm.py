...
class EDM(torch.nn.Module):
    def __init__(
            ...
            n_dims: int,
            timesteps: int = 1000,
            noise_schedule='learned', # polynomial_2
        ...
        self.in_node_nf = in_node_nf # 8 
        self.n_dims = n_dims # 3
        self.T = timesteps # 500
        self.norm_values = norm_values # [1, 4, 10] 
        self.norm_biases = norm_biases # (None, 0.0, 0.0)
    ...
    def forward(self, x, h, node_mask, fragment_mask, linker_mask, edge_mask, context=None):
        ...
        delta_log_px = self.delta_log_px(linker_mask).mean() # 0
        ...
        z_t = xh * fragment_mask + z_t * linker_mask # [batchsize, 33, 11]
    ...
    def sample_chain(self, x, h, node_mask, fragment_mask, linker_mask, edge_mask, context, keep_frames=None):
        ...
        # # 额外加的判断
        # if torch.any(torch.isnan(xh)) or torch.any(torch.isnan(z)):
        #     print('in edm.py')
        #     raise utils.FoundNaNException(xh, z)
