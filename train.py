...
from src.ddpm import DDPM, Trainer
from src.utils import disable_rdkit_logging, Logger

# nohup python -W ignore train.py --config train_given_anchors.yml > config_train.log 2>&1 &
...
def main(args):
    ...
    trainer = Trainer(n_epochs=args.n_epochs, data_path=args.data, train_data_prefix=args.train_data_prefix,
        val_data_prefix=args.val_data_prefix, batch_size=args.batch_size, accelerator=args.device, gpu_ids=args.gpu_ids, )
    print('Start training')
    trainer.train(model=ddpm, checkpoints_dir=checkpoints_dir, ckpt_path=last_checkpoint)
